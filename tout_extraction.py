...
url_principal="https://books.toscrape.com/" # on entre dans le site books.toscrape.com pour examiner et récupérer les liens
#vers toutes les catégories
rep_principal=requests.get(url_principal)
principal=BeautifulSoup(rep_principal.content,'html.parser')
#on recupère l'url de toutes les catégories
cat_principal=principal.findAll('ul',class_="nav nav-list") # nous avons ici le résultat qui est une liste à 1 seul élt
li=cat_principal[0]
il=li.findAll('li')#recherche des balise li dans l'élt précédent. ceci retourne une liste que nous appelerons il
itil=[]
for z in il:
    k=z.find('a') #pour chaque elt de il, extraire la balise "a"
    q=k['href']    # chaque elt ici étant un dictionnaire, on recupère la valeur de la clé 'href'
    itil.append(q)  # liste des href recuperé
#print(itil)
lien_de_categorie=[]
for elt in itil:
    ref=url_principal+elt
...
for lien_total in lien_de_categorie:
    ...
    url_cat = lien_total  # on recupère l'url sur le
    # catologue
    next_url = url_cat
    iteration = 1
    while next_url != "":
        ...
        cat = pag.findAll('li',
                          class_="col-xs-6 col-sm-4 col-md-3 col-lg-3")  # l'idée est de sortir ayant récupèrer tous
        # les lien sur tous les élements
        ...
        for link in lien:
            ...
            title = page.findAll(
                class_="col-sm-6 product_main")  # recupération des éléments de l'attribut class_="col-sm-6 product_main"
            ...
            des = page.findAll('p')
            description = des[
                3].text  # 'p' est le seul critère de selection que j'ai trouvé et je trouve que mon elt recherché est
